...
if (isset($_SESSION['userid'])) {  // checks to see if already logged in
    ...
    try {
    $usr = login(dbconnect(), $_POST["email"]);  // does it here to ensure we can use parts of the data if successful

    if ($usr && password_verify($_POST["password"], $usr["password"])) { // verifies the password is matched
        audtitor(dbconnect(),$usr["userid"],"LGI", "User has successfully logged in");  // audit logs the login
        ...
        header("location:wishlist.php");  //redirect on success
...
echo "<h2> Wish List - Login</h2>";  # sets a h2 heading as a welcome
